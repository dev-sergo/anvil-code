...
**Статус проекта**: 🟡 v1.32-d Phase B+D landed — llama-swap backend swap. `LlamaSwapClient` (OpenAI-compat `/v1/chat/completions` + SSE streaming + tool calls + inline JSON fallback + `/v1/embeddings`) живёт рядом с `OllamaClient` под `LLM_BACKEND` env switch. `ModelBackend` interface + factory разделяют транспорт от ролей. Per-role aliases: `coder`/`qwen3`/`embed`. Nomic-v1.5 task prefixes (`search_query:` / `search_document:`) wired в `packages/rag/` — backend-agnostic, ~5-10% similarity win на обоих путях. 426/426 unit-tests (+34: 31 backend + 3 prefix). **Pending:** Phase E (bench L1.1+L4.1+scale на llama-swap baseline), Phase F (flip `LLM_BACKEND=llamacpp` default).  
