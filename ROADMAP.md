...
**Статус:** 🟢 v1.65d done (2026-05-20). **trpc bench: 5/6 (83%) ✅ — новый рекорд Qwen3, равно Gemma v1.43 peak.** T1✅ T2✅(впервые) T3❌ T4✅ T5✅ T6✅(add_type_member). Ключевой фикс: TestRunner 60s→120s. v1.65d: `add_type_member` поддержка intersection types + FEATURE_SPEC ADD OPTION rule. Sandbox восстановлен (новый initial commit `907dbae` вместо `24ce9fa`). `add_type_member` — AST-инструмент для добавления членов в interface/type alias. Прямой fix для T6 noop (DataLoaderOptions 900-строчный файл). Плюс v1.65a Reviewer leniency, v1.64 Repo memory.
