...
#### v1.32-a.4 — L4.1 robustness ×5 (✅ запущено, без code changes)

**Цель:** v1.32-a.3 закрыл L4.1 на single run. Эта итерация measures **stability** — детерминирован ли operator-grade bug-fix workflow на 5 последовательных runs идентичного bug-inject baseline.

**Результаты ×5:**

| Run | Commit | Wall | Diff quality |
|---|---|---|---|
| #1 | ✓ b81096f | ~58 min | structural noise (duplicate `users.set/return` после первого return — unreachable code, valid TS) |
| #2 | ✓ f159d20 | ~6 min | minor style (lost indent на `create`, removed trailing comma) |
| #3 | ✓ 65e59dd | ~6 min | style noise (6-space indent, dropped User type) |
| #4 | ✓ 28cabdc | **~5 min** | **byte-perfect минимальный diff** |
| #5 | ✓ f1074e5 | ~17 min | minor style (5 vs 4 space indent, extra blank line) |

**Aggregate:**
- **Commit-landed rate: 5/5 (100%)** ✓
- **Bug semantically fixed: 5/5** — все runs добавляют `createdAt` + убирают `as User` cast
- **Fixer-bail rate: 0/5** (vs v1.32-a.2's 100% bail on 2 runs)
- **Destructive failures: 0/5**
- Wall-time: median 6 min, mean 18 min (skewed by run #1 outlier — Coder loop pathology), p90 ≈17 min
- Diff quality: 1/5 byte-perfect, 3/5 minor style noise (operator-cleanable seconds), 1/5 structural noise (dead code, не destructive), 0/5 destructive

**Variance moved to the QUALITY layer, не CORRECTNESS layer.** Pipeline reliable; LLM output style varies. Operator review остаётся, но всегда получает working fix.

**v1.31.2 → v1.32-a.4 arc decisively complete:**

| Iter | Result | Severity |
|---|---|---|
| v1.31.2 | broken commit shipped | HIGH |
| v1.32-a | commit_skipped | LOW-MED |
| v1.32-a.1 | fix in tree, not committed | LOW |
| v1.32-a.2 ×2 | Fixer bailed | n/a |
| v1.32-a.3 | first end-to-end commit | CLOSED |
| **v1.32-a.4 ×5** | **5/5 commits land, 0 destructive** | **STABLE** ✓ |

**Remaining imperfections (independent от a.4 specifically):**
- Coder loop pathology (run #1 took 58 min — same v1.32-a.1 brace-balance retry issue) → **v1.32-a.5 candidate (~2-3h)**: detect repeated identical tool calls, break early с system reminder
- Cosmetic style noise в 4/5 runs → **v1.32-a.6 candidate (~1h)**: `prettier --write` post-step в orchestrator
- Coder produces cosmetic edit на routes/users.ts даже когда real fix elsewhere — discussed в v1.32-a.3, не блокер

**Phase 3 closure now realistic.** Next priority order:
1. **v1.32-a.5** Coder loop-pathology guard (high value for daily-driver wall-time)
2. **v1.32-a.6** prettier post-step (cheap quality bump)
3. **v1.32-c** sub-agents (Phase 3 architectural closure, ~1 неделя)
...
#### v1.32-d — llama-swap backend swap (🟡 Phase B+D landed, Phase E/F pending)

**Цель:** уйти от Ollama-зависимости в `packages/model-router/`; перейти на operator's llama-swap proxy (OpenAI-compatible API). Архитектурно: `ModelBackend` interface + factory; `LlamaSwapClient` живёт рядом с `OllamaClient`. Single endpoint на `LLM_URL`, per-role MODEL ALIAS routing. **Без grammar-constrained tool calls** — отдельная итерация v1.33.

**Operator's setup verified 2026-05-02** (см. [docs/llama-api-reference.md](docs/llama-api-reference.md)): llama-swap прокси на `172.20.10.4:8080` с aliases `coder`/`qwen-coder-long`/`qwen`/`qwen3`/`gemma`/`llama70`/`embed` (nomic-embed-text-v1.5, 768 dim).

**Phase B (✅ landed) — backend abstraction + LlamaSwapClient:**
- [x] [packages/shared/src/config.ts](packages/shared/src/config.ts) — `llmBackend`, `embedBackend`, `llamacpp.{url,modelLarge,modelLargeLong,modelSmall,modelEmbed}` env vars + validateConfig
- [x] [packages/model-router/src/shared-helpers.ts](packages/model-router/src/shared-helpers.ts) — extracted `extractInlineToolCalls` + `findMatchingBrace` (oба клиента переиспользуют — qwen-family models emit tool calls inline в content)
- [x] [packages/model-router/src/backend.ts](packages/model-router/src/backend.ts) — `ModelBackend` interface + `createChatBackend()` / `createEmbedBackend()` / `effectiveEmbedBackendKind()` / `effectiveEmbedModel()` factories
- [x] [packages/model-router/src/llamaswap-client.ts](packages/model-router/src/llamaswap-client.ts) — chat / chatStream (SSE с `data: [DONE]`) / chatWithTools (OpenAI shape `tool_calls[i].function.arguments` JSON-string + parse, inline JSON fallback) / embed / embedBatch / healthCheck
- [x] [packages/model-router/src/router.ts](packages/model-router/src/router.ts) — factory-driven, per-backend model namespace selection
- [x] **+31 unit-теста** (5 файлов): chat (7), stream SSE (6), tools OpenAI shape + inline fallback (6), embed single+batch (6), backend-routing factory + hybrid mode (6)

**Phase D (✅ landed) — rag/ prefix wiring + factory integration:**
- [x] [packages/rag/src/graph-retriever.ts](packages/rag/src/graph-retriever.ts) — `OllamaClient` → `createEmbedBackend()` factory; `embedWithCache(text, mode: 'query' | 'document')`; cache key включает model label + mode (избегает collision query/doc embeddings + Ollama↔llama-swap инвалидация)
- [x] **Nomic-embed-text-v1.5 task prefixes** wired backend-agnostic: `search_query: <text>` на retrieval path, `search_document: <text>` на indexing path. Без них — деградация similarity на 5-10%
- [x] **+3 unit-теста**: search_document на indexing, search_query на retrieval, cache key separation by mode

**Build + tests:** 426/426 green (+34 vs v1.32-a.5's 392), 12/12 packages build clean.

**Phase A (probe), Phase C (tool-calling parity)** — done implicitly (operator probed `embed` alias; parity достигается через shared `extractInlineToolCalls`).

**Phase E (pending) — bench L1.1+L4.1+scale на llama-swap baseline:**
- [ ] Pre-bench: reset `data/vectors/* data/graphs/*`, reindex с новым prefix-aware embed
- [ ] L1.1 ×3 (sandbox), L4.1 ×3 (sandbox), scale ×1 (`rag-system-target`)
- [ ] Run-file: [docs/benchmarks/runs/2026-05-02-v1.32-d-llamacpp-backend.md](docs/benchmarks/runs/2026-05-02-v1.32-d-llamacpp-backend.md) (skeleton готов)

**Phase F (pending) — flip default `LLM_BACKEND=llamacpp`** после Phase E зелёного.

**Detailed design:** [docs/designs/v1.32-d-llamacpp-backend.md](docs/designs/v1.32-d-llamacpp-backend.md)

#### v1.32-a.6 — Prettier post-step (✅ реализовано)

**Цель:** v1.32-a.4 5/5 commits показали что 4/5 runs landed с **cosmetic style noise** (indent depth, blank lines, dropped trailing commas) — operator-cleanable seconds, но всё же diff noise. Запустить `prettier --write` на `writtenFiles` после validation pass и до commit, чтобы стиль соответствовал project'овой настройке prettier'а. **Cosmetics-only — никогда не блокирует commit.**

- [x] [packages/safe-exec/src/prettier-runner.ts](packages/safe-exec/src/prettier-runner.ts):
  - `isPrettierConfigured(root)` — детект через `.prettierrc*` files / `prettier.config.{js,cjs,mjs}` / `package.json` "prettier" field / prettier in deps/devDeps
  - `PrettierRunner.run(paths)` — фильтр formattable extensions (ts/tsx/js/jsx/mjs/cjs/json/yaml/md/html/css/scss/vue/svelte) → spawn local `node_modules/.bin/prettier --write`. **Без global / npx --yes fallback** — иначе риск reformat'a с разной версией. Skip path: 'no files' / 'no prettier config' / 'no formattable files' / 'prettier not installed in target project'
  - PrettierResult.success=true даже когда skipped; false только при actual crash → orchestrator логирует warning, commit всё равно идёт
- [x] [packages/agents/src/orchestrator.ts](packages/agents/src/orchestrator.ts) `runTask`: после `validation` → перед `git.commitChanges` — `await this.prettier.run(writtenFiles)`. Никаких task event'ов (cosmetic step), просто log info на success / warn на crash
- [x] **+15 unit-тестов**: 8 detection signals (configs, package.json fields), 5 skip paths, 2 actual run (fake prettier binary в node_modules/.bin) — verify args correctness + non-zero exit handling. **441/441** общая
- [x] [.env.example](.env.example) — N/A (no new env vars; prettier config — domain target project'a)

**Trade-off:** prettier добавляет ~1-3s на commit когда configured. Acceptable; alternative — operator вручную `prettier --write` после каждой задачи.

**Effect (ожидаемый, не bench'нут отдельно):** v1.32-a.4 1/5 byte-perfect → ожидается **5/5 byte-perfect** на projects где prettier настроен. Validate естественно через bench v1.32-c (sub-agents) — prettier работает на любом backend.
